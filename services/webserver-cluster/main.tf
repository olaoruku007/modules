...
resource "aws_autoscaling_group" "pro-asg" {
  ...
  tag {
    key                 = "Name"
    value               = "${var.cluster_name}-terraform-pro-asg"
...
resource "aws_security_group" "pro-sg" {
  name   = "${var.cluster_name}-terraform-pro-sg"
  vpc_id = data.aws_vpc.default.id
  ingress {
...
resource "aws_lb" "terraform-alb" {
  name               = "${var.cluster_name}-t-pro-asg-alb"
...
resource "aws_security_group" "alb" {
  name   = "${var.cluster_name}-terraform-sg-alb"
  vpc_id = data.aws_vpc.default.id
...
resource "aws_lb_target_group" "asg" {
  name     = "${var.cluster_name}-t-asg-tg"
  port     = var.HTTP_port
  protocol = "HTTP"
  vpc_id   = data.aws_vpc.default.id
